...
export default function ChemicalPipelinePage() {
  ...
  const [singleSmiles, setSingleSmiles] = useState<string>("")
  ...
  const handleTabChange = (value: string) => {
    ...
    if (value === "single-smiles") {
      setUploadedFile(null) // Clear file when switching to single SMILES
    } else if (value === "file-upload") {
      setSingleSmiles("") // Clear SMILES input when switching to file upload
    }
  ...
  const handleProcess = async () => {
    ...
    if (activeTab === "file-upload" && (!uploadedFile || !selectedModel)) {
      setError("Please select a file and model before processing.")
      return
    }
    
    if (activeTab === "single-smiles" && (!singleSmiles.trim() || !selectedModel)) {
      setError("Please enter a SMILES string and select a model before processing.")
      return
    }
    ...
    console.log("SMILES:", singleSmiles)
    ...
    try {
      ...
      if (activeTab === "file-upload") {
        ...
        // For single SMILES, create a temporary CSV-like data
        const csvData = "SMILES\n" + singleSmiles
        const blob = new Blob([csvData], { type: "text/csv" })
        formData.append("file", blob, "single_smiles.csv")
      }
              ...
              <CardContent className="space-y-6">
                ...
                <Button
                  ...
                  disabled={isProcessing || 
                    (activeTab === "file-upload" && (!uploadedFile || !selectedModel)) ||
                    (activeTab === "single-smiles" && (!singleSmiles.trim() || !selectedModel))
                  }
                  className="w-full"
                >
                  {isProcessing ? (
