...
def generate_structure_svg(smiles, atom_id):
    """Generate a Base64-encoded SVG image for a molecule with a highlighted atom."""
    if pd.isna(smiles):
        return ""
    ...
    # Encode SVG to Base64 for safe CSV storage
...
def apply_rule_based_logic(input_csv, output_csv):
    ...
    for _, row in df.iterrows():
        ...
        output["Structure"] = generate_structure_svg(smiles, atom_id)
    ...
    bars = plt.bar(summary.index, summary.values, color=['red', 'orange', 'yellow', 'green'])
    plt.title("Nitrosamine Likelihood Prediction Summary")
    ...
    plt.tight_layout()
    plt.savefig(output_csv.parent / "likelihood_summary.png")
    plt.close()
    logger.info(f"Likelihood summary plot saved to {output_csv.parent / 'likelihood_summary.png'}")
...
#         ("Tertiary nitrogen (at atom ID)",
#          lambda mol, row: (
#              pd.notna(row["Recovered_Atom_ID"]) and
#              (atom := mol.GetAtomWithIdx(int(row["Recovered_Atom_ID"]))).GetAtomicNum() == 7 and
#              atom.GetDegree() == 3
#          ), -4, True),
...
#         ("Tertiary amine with pKa gt 9.5",
#          lambda mol, row: (
#              pd.notna(row["Recovered_Atom_ID"]) and
#              pd.notna(row.get("pKa_25")) and # Added check for pKa_25 presence
#              (atom := mol.GetAtomWithIdx(int(row["Recovered_Atom_ID"]))).GetAtomicNum() == 7 and
#              atom.GetDegree() == 3 and
#              atom.GetTotalNumHs(includeNeighbors=True) == 0 and
#              float(row["pKa_25"]) > 9.5
...
#         ("Secondary amine with pKa gt 9.5",
#          lambda mol, row: (
#              pd.notna(row["Recovered_Atom_ID"]) and
#              pd.notna(row.get("pKa_25")) and # Added check for pKa_25 presence
#              (atom := mol.GetAtomWithIdx(int(row["Recovered_Atom_ID"]))).GetAtomicNum() == 7 and
...
#         try:
#             atom_id = int(row.get("Recovered_Atom_ID", -1)) if pd.notna(row.get("Recovered_Atom_ID")) else -1
...
#                 else:
#                     matches = mol.GetSubstructMatches(pattern)
#                     matched = any(atom_id in match for match in matches) if atom_id >= 0 else False
...
#             except Exception as e:
#                 rule_hits[desc] = 0
#                 logger.warning(f"Rule {desc} failed: {e}")
...
#         output = row.copy()
#         output["Matched_Rules"] = ";".join(matched_rules) if matched_rules else ""
#         output["Score"] = score
#         output["Likelihood"] = likelihood
...
#     import sys
#     if len(sys.argv) != 3:
#         print("Usage: python rule_based_logic.py <input_csv> <output_csv>")
#         sys.exit(1)